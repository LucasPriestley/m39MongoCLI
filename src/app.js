...
const app = async (yargsObj) => {
  ...
  if (yargsObj.create) {
    ...
    console.log("R");
    const results = await collection.find({title: yargsObj.title, actor: yargsObj.actor}).toArray();
    console.log(results);
    //list items from database
  } else if (yargsObj.update) {
    console.log("U");
    const update = new UpdateMovie(yargsObj.title, yargsObj.newTitle, yargsObj.newActor);
